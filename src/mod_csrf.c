...
{
    csrf_opts * const opts = calloc(1, sizeof(opts));
    force_assert(opts);
    ...
    if (opts->secret) {
        if (buffer_string_length(opts->secret) < SECRET_SIZE) {
            log_error(srv->errh, __FILE__, __LINE__,
                    ...
                    const char * const extra, const uint32_t elen)
{
    ...
    buffer_clear(tb);
    buffer_append_base64_encode_no_padding(tb, msg, mlen + opts->dlen,
                                           BASE64_URL);
    ...
    if (!buffer_string_is_empty(opts->xsrfcookie)) {
        /* shift token in tb to construct xsrfcookie value in tb */
        const uint32_t nlen = buffer_string_length(opts->xsrfcookie);
        const uint32_t tlen = buffer_string_length(tb);
        buffer_string_set_length(tb, nlen + 1 + tlen);
        memmove(tb->ptr+nlen+1, tb->ptr, tlen);
        memcpy(tb->ptr, opts->xsrfcookie->ptr, nlen);
        tb->ptr[nlen] = '=';
                         ...
                         const char * const extra, const uint32_t elen)
{
    ...
    const uint32_t mlen = 9;
    if (buffer_string_length(tb) != mlen + opts->dlen)
        return TOKEN_CHECK_INVALID;
    ...
    return opts->hmac_fn(digest, CONST_BUF_LEN(opts->secret),
        ...
        && http_auth_const_time_memeq(input, (char *)digest, opts->dlen)
...
mod_csrf_same_origin (const request_st * const r, const buffer * const b)
{
    ...
    const uint32_t slen = buffer_string_length(&r->uri.scheme);
    const uint32_t alen = buffer_string_length(&r->uri.authority);
...
URIHANDLER_FUNC(mod_csrf_uri_handler)
{
    ...
    if (!buffer_string_is_empty(opts->sesscookie)) {
        ...
        if (NULL != vb) {
            ...
            if (NULL != s) {
                s += buffer_string_length(opts->sesscookie);
                while (*s == ' ' || *s == '\t') ++s; /* skip WS */
    ...
    if (!buffer_string_is_empty(opts->xsrfcookie)
        ...
        if (NULL != vb) {
            ...
            if (NULL != s) {
                s += buffer_string_length(opts->xsrfcookie);
                while (*s == ' ' || *s == '\t') ++s; /* skip WS */
