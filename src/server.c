...
static void sigaction_handler(int sig, siginfo_t *si, void *context) {
	...
	case SIGHUP:
		 ...
		 * infinite loop when parent forwards signal to process group
		 */
...
static void signal_handler(int sig) {
	...
	case SIGHUP:
	     if (!forwarded_sig_hup) handle_sig_hup = 1;
	     else forwarded_sig_hup = 0;
	     break;
	case SIGCHLD:  break;
	}
}
...
__attribute_cold__
__attribute_noinline__
static void server_free(server * const srv) {
	size_t i;
...
__attribute_noinline__
static int
lighttpd_process_sighup (server * const srv) {
	handler_t r;
	...
	if ((r = plugins_call_handle_sighup(srv)) != HANDLER_GO_ON) {
...
__attribute_noinline__
static void
lighttpd_periodic_jobs (server * const srv, const time_t min_ts) {
#if 0
	int cs = 0;
#endif
...
__attribute_noinline__
static void
lighttpd_check_waiting_fds (server * const srv) {
	/* check the fdwaitqueue for waiting fds */
	connection *con;
	int free_fds = srv->max_fds - srv->cur_fds - 16 + 1;

	while (--free_fds > 0
		&& NULL != (con = fdwaitqueue_unshift(srv, srv->fdwaitqueue))) {
		connection_state_machine(srv, con);
		--srv->want_fds;
	}
}
...
__attribute_noinline__
static void
lighttpd_process_overload_condition (server * const srv) {
	unsigned int i;
...
__attribute_noinline__
static int
lighttpd_srv_setup (server * const srv, int argc, char **argv) {
	unsigned int i;
	int print_config = 0;
	int test_config = 0;
	int i_am_root;
	...
	if (NULL == srv) {  /* server_init() failed to calloc() */
	...
	while(-1 != (o = getopt(argc, argv, "f:m:hvVDpt"))) {
		...
		case 'v': show_version();  server_free(srv); exit(0);
		case 'V': show_features(); server_free(srv); exit(0);
		case 'h': show_help();     server_free(srv); exit(0);
		default:
			show_help();
			server_free(srv);
			return -1;
...
# endif
	...
	srv->gid = getgid();
	srv->uid = getuid();
	last_sigterm_info.si_uid = srv->uid;
	last_sigterm_info.si_pid = getpid();
...
__attribute_noinline__
static int
lighttpd_fdevent_init (server * const srv) {
	/* should be called after fork(), due to some fdevent mechanisms */
	unsigned int i;
...
__attribute_noinline__
static int
lighttpd_fdevent_loop (server * const srv) {
	connections * const joblist = srv->joblist;
	unsigned int ndx;
	handler_t r;
	connection *con;
	time_t min_ts;
	const int fd_hiwat = srv->max_fds * 9 / 10;

	/* main-loop */
	while (!srv_shutdown && !(graceful_shutdown && srv->conns->used == 0)) {

		if (handle_sig_hup && 0 != lighttpd_process_sighup(srv)) {
			return -1;
		}

		/* periodic jobs (e.g. check for connection idle timeout) */
#ifdef USE_ALARM
		if (handle_sig_alarm) {       /* a new second */
			handle_sig_alarm = 0; /* reset notification */
#endif
			min_ts = time(NULL);
			if (min_ts != srv->cur_ts) {
				lighttpd_periodic_jobs(srv, min_ts);
			}
#ifdef USE_ALARM
		}
#endif

		/* check for waiting fds */
		if (srv->want_fds) {
			lighttpd_check_waiting_fds(srv);
		}

		/* overload condition check */
		if (srv->cur_fds + srv->want_fds > fd_hiwat || /* out of fds */
		    srv->conns->used >= srv->max_conns || /* out of connections */
		    graceful_shutdown || /* graceful_shutdown */
		    srv->sockets_disabled) { /* already overloaded */
			lighttpd_process_overload_condition(srv);
		}

		/* poll for and dispatch ready events */
		fdevent_poll_dispatch(srv->ev, srv, 1000);

		/* queued jobs */
		for (ndx = 0; ndx < joblist->used; ndx++) {
			con = joblist->ptr[ndx];
			connection_state_machine(srv, con);
			r = plugins_call_handle_joblist(srv, con);
			if (r != HANDLER_FINISHED && r != HANDLER_GO_ON)
				log_error_write(srv, __FILE__, __LINE__, "d", r);
			con->in_joblist = 0;
		}
		joblist->used = 0;
...
__attribute_cold__
__attribute_noinline__
static void lighttpd_srv_cleanup (server * const srv, const int rm_pid_file) {
	...
	/* parent or single process removes pid file */
	if ((0 == srv->srvconf.max_worker || rm_pid_file)
	    && srv->srvconf.pid_file != NULL
	    && srv->srvconf.pid_file->used
	    && srv->srvconf.changeroot->used == 0
	    && 0 != unlink(srv->srvconf.pid_file->ptr)
	    && errno != EACCES && errno != EPERM) {
		log_error_write(srv, __FILE__, __LINE__, "sbds",
				"unlink failed for:", srv->srvconf.pid_file,
...
int main (int argc, char **argv) {
	...
	if (0 != lighttpd_srv_setup(srv, argc, argv))
		return -1;

#ifdef HAVE_FORK
	if (srv->srvconf.max_worker > 0) {
		int num_childs = srv->srvconf.max_worker;
		pid = -1;
		/* watcher loop (parent) creates workers (forked children) */
		while (!srv_shutdown && !graceful_shutdown) {
			if (num_childs > 0) {
				if (0 == (pid = fork()))
					break;  /* child handled outside loop */
				else if (pid > 0)
					--num_childs; /* parent spawned child */
				else
					sleep(1);     /* parent sleep, retry */
			} else if (-1 != wait(NULL)) {
				++num_childs;         /* parent reaped child */
			} else if (handle_sig_hup) {
				/* EINTR == errno and signalled with SIGHUP.
				 * reopen error log (to allow log rotation)
				 * (do not exit parent if log cycling fails) */
				lighttpd_process_sighup(srv);
				/* forward sighup to all procs in process group
				 * (including ourself; detect using flag) */
				if (!forwarded_sig_hup) {
					forwarded_sig_hup = 1;
					kill(0, SIGHUP);
				}
	...
	if (0 != pid) { /* watcher (parent) kill all children */
		if (graceful_shutdown)
			kill(0, SIGINT);
		else if (srv_shutdown)
			kill(0, SIGTERM);
	}
	else	/* event loop (child or single process) */
#endif
		if (0 == (rc = lighttpd_fdevent_init(srv)))
			rc = lighttpd_fdevent_loop(srv);

	lighttpd_srv_cleanup(srv, pid);
	return rc;
}
